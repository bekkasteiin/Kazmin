...
class TeamKpiInfo extends StatefulWidget {

  @override
  _TeamKpiInfoState createState() => _TeamKpiInfoState();
}

class _TeamKpiInfoState extends State<TeamKpiInfo> {
  @override
  ...
  Widget kpiPanel({TeamKpiModel model}) {
    return Column(
      children: [
        Padding(
          ...
          child: Card(
            child: ExpansionTile(
                title: const Text('Информация о сотруднике', style: TextStyle(fontWeight: FontWeight.w600),),
                children: [
                  ListTile(
                  ...
                  ListTile(
                    ...
                    subtitle:
                    Text(model?.selected?.assignedPerson?.assignments?.first?.organizationGroup?.instanceName ??''),
                  ),
                  ListTile(
                    title: const Text('Статус'),
                    subtitle: Text(model?.selected?.stepStageStatus ??''),
                  ),
                  ListTile(
                    title: const Text('Дата с'),
                    subtitle: Text(formatShortly(model?.selected?.startDate)),
                  ),
                  ListTile(
                    title: const Text('Дата по'),
                    subtitle: Text(formatShortly(model?.selected?.endDate)),
                  ),
                  ListTile(
                    title: const Text('Дата приема на работу'),
                    subtitle: Text(formatShortly(model?.selected?.assignedPerson?.person?.hireDate)),
                  ),
                ],
            ),
          ),
  ...
  Widget buttonPanelStartBpm({TeamKpiModel model}){
    return Container(
  ...
  Widget goalsPanel({@required TeamKpiModel model}){
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(15.0),
        child: Column(
