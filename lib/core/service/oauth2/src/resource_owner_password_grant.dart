...
const String fName = 'lib/core/service/oauth2/src/resource_owner_password_grant.dart';
...
Future<Client> resourceOwnerPasswordGrant(
  ...
  if (additionalHeaders != null && additionalHeaders.isNotEmpty) headers.addAll(additionalHeaders);

  if (scopes != null && scopes.isNotEmpty) body['scope'] = scopes.join(delimiter);

  if (httpClient == null) httpClient = new http.Client();
  // log('-->> $fName, resourceOwnerPasswordGrant ->> httpClient: ${httpClient.toString()}');
  ...
  return new Client(credentials, identifier: identifier, secret: secret, httpClient: httpClient, onCredentialsRefreshed: onCredentialsRefreshed);
}
